...
* **Status:** **FIXED.** `SocketEvents` is now the production-visible internal class in `win32/socket_events.h:30-90`; `reconcile()` retains registrations for live handles but calls `WSAEventSelect(fd, event, FD_READ)` for each one (`win32/socket_events.h:59-82`), relying on Winsock's documented re-recording of `FD_READ` for queued datagrams so a recycled raw handle is armed for its new socket. It preserves event creation, error cleanup, and the destructor's deregistration semantics. `win32/test_socket_events.cc:31-80` creates real loopback UDP sockets, requires a recycled raw `SOCKET` value, sends a datagram, reconciles again before enumerating, and observes `FD_READ` through `WSAEnumNetworkEvents`; it is RED before the re-arm and GREEN afterward on native windows-11-arm CI.
...
* **Status:** **FIXED** (commits `4fe02e3` and follow-up). `Connection::Socket`'s Win32 ctor passes a `BOOL FALSE` input to `WSAIoctl(SIO_UDP_CONNRESET)` (`src/network/network.cc:220-230`) to disable reset notifications, but logs and continues if a provider rejects this optional ioctl. `receive_should_continue()` (`src/network/network.cc:557-565`) makes `WSAECONNRESET`, `WSAEWOULDBLOCK`, and `WSAEMSGSIZE` nonfatal for both `recv()` and `recv_from()` (`src/network/network.cc:565-605`). `win32/test_loopback.cc:expect_no_connreset()` (`:140-190`) sends to a just-closed loopback UDP port and polls the tested connection until its ICMP response is consumed without error; runs on native windows-11-arm CI (the Linux cross-compile image only compiles it).
