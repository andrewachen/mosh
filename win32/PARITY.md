...
* **On pinning the proxy options:** no rationale was recorded where they were introduced (`aa41fc8`), and two this document has since offered do not survive scrutiny. Keeping the parsed stream stable is not it — those options select the transport under the SSH session and leave the remote's stdout channel alone, unlike a PTY. Nor does the pin follow from missing discovery: a route through a bastion can still announce an address the client routes to directly, so refusing it rejects sessions that would work. What remains is the honest description — plain SSH only, a support restriction deliberately broader than the reachability gap that motivates it, and one that would have to be lifted for any mode that supplies its own endpoint. `-S none` belongs with it: an existing control master could otherwise carry the session over a route this invocation means to refuse.
* **Fix, if the decision is revisited:** an override needs nothing secret on the command line — see the S1 record, which separates endpoint selection from key transport. Its shape is not settled here, and the topologies differ: an address rewrite suffices where NAT preserves the port, while a translated port needs the client-visible port too. `mosh-server new -p PORT[:PORT2]` already exists for requesting the listening port (`src/frontend/mosh-server.cc:234`, forwarded by upstream as `--port`, `scripts/mosh.pl:159`); this bootstrap simply does not expose it. Implementing upstream's `local` or `proxy` discovery is the other path and keeps the interface unchanged.
