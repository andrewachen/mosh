...
* **Consequence:** two distinct failures, which should not be described as one. Behind NAT or a load balancer, `ssh` connects normally, `mosh-server` starts, and the address it reports is the server's own private one — so the UDP session times out with no diagnostic naming the address it tried, and a remote server is left running. For a host reachable *only* through a jump, the pinned options mean `ssh` never connects at all, so the failure is immediate and there is no orphaned server. The retired positional endpoint form was the only override for the first case, and it is gone.
* **Class:** `POLICY`, medium. Out of scope by decision: the supported case is a host the client can route to directly, and no discovery work is scheduled. Recorded as a finding because it is a user-visible behavioral difference from upstream, not because it is planned. Revisit if a supported deployment lands behind NAT or a bastion; nothing else should reopen it.
* **On pinning the proxy options:** their rationale is not recorded where they were introduced (`aa41fc8`), and the one this document previously gave — keeping the parsed stream stable — does not hold: those options select the transport underneath the SSH session and leave the remote's stdout channel alone, unlike a PTY. What they do achieve is making an unusable endpoint fail loudly. Without a discovery method, a proxied SSH would succeed and hand back an address the client cannot route to; refusing the route converts a silent UDP timeout into an immediate `ssh` error. Keep them pinned only as long as discovery is absent — they are a consequence of that gap, not an independent policy.
* **Fix, if the decision is revisited:** an address-only override needs nothing secret on the command line — see the S1 record, which separates endpoint selection from key transport. It must be address-only: `mosh-server` picks its UDP port dynamically and announces it in the `MOSH CONNECT` reply, so a caller cannot know a port in advance, and overriding the announced one would aim the session at a port nothing is listening on. Supporting a translated port would first require a server-side port request, which does not exist. Implementing upstream's `local` or `proxy` discovery is the other path and keeps the interface unchanged.
