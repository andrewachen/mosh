...
* **Status:** **FIXED.** `SocketEvents` is now the production-visible internal class in `win32/socket_events.h:30-88`; `reconcile()` retains registrations for live handles but calls `WSAEventSelect(fd, event, FD_READ)` for each one (`win32/socket_events.h:59-80`), so a recycled raw handle is armed for its new socket. It preserves event creation, error cleanup, and the destructor's deregistration semantics. `win32/test_socket_events.cc:31-79` creates real loopback UDP sockets, requires a recycled raw `SOCKET` value, reconciles it, and observes its `FD_READ` record through `WSAEnumNetworkEvents`; it is RED before the re-arm and GREEN afterward on native windows-11-arm CI.
