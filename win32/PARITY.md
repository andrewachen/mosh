...
* **Upstream:** the wrapper discovers a client-visible address before starting the session — the default `proxy` method captures it through a client-side `ProxyCommand`, and `local` resolves the target client-side (`scripts/mosh.pl:364`) — and honors whatever SSH route the user has configured.
...
* **Consequence:** behind NAT, a load balancer, or a jump host, the reported address is the server's own and often private. `ssh` connects, `mosh-server` starts, and the UDP session then times out with no diagnostic naming the address it tried. The retired positional endpoint form was the only way to override this, and it is gone.
* **Class:** `POLICY`, medium. Out of scope by decision: the supported case is a host the client can route to directly, and no discovery work is scheduled. Recorded as a finding because it is a user-visible behavioral difference from upstream, not because it is planned.
* **Fix, if the decision is revisited:** an address-and-port override needs nothing secret on the command line — see the S1 record, which separates endpoint selection from key transport. Implementing upstream's `local` or `proxy` discovery is the other path and keeps the interface unchanged.
* **Related:** A21 covers the missing exit diagnostics. This is where they would matter most: a timeout that named the address it tried, and said that proxied topologies are unsupported, would turn an unexplained hang into an answer.
