...
* **Verification:** The `deadline-wedged-reader` mode asserts that deadline cleanup detached the live reader rather than destroying it (`win32/test_console_lifecycle.cc:1053-1109`). CI runs the mode (`.github/workflows/windows.yml:188`).
...
* **Verification:** The `deadline-throw-unwind` lifecycle mode makes `run()` throw after deadline teardown, publishes a tick immediately before its by-value session is destroyed, and bounds only that destruction interval (`win32/test_console_lifecycle.cc:1112-1172`). CI runs the mode (`.github/workflows/windows.yml:190`).
...
* **Verification:** `win32/test_terminal_width.cc` asserts the astral width classification across all four width classes, unit-tests the `mosh_mbrtoc32` malformed-input contract (overlong forms, encoded surrogates, truncation, NUL), and drives a 4-byte UTF-8 emoji through the real `UTF8Parser` into an `Emulator`, requiring one wide cell with the original bytes and a two-column cursor advance. `win32/test_prediction_width.cc` feeds the same emoji through `PredictionEngine::new_user_byte` and requires that no narrow prediction cell is installed (the pre-fix truncation to 0xF600 would have predicted one), with an ASCII control proving ordinary prediction still works. CI runs both natively on ARM64. A RED-probe branch that reverted only the decode/encode to raw `mbrtowc`/`wcrtomb` failed the emulator assertion on native ARM64 (`get_cursor_col() == 2`), confirming the test discriminates the fix.
