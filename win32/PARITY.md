...
* **Status:** **FIXED.** `MoshCore::on_readable()` now passes the enumerated socket to its receive pass (`win32/mosh_core.cc:198-227`, `win32/mosh_core.cc:362-366`), which preserves the existing 32-datagram budget and error handling. `Transport::recv(intptr_t)` uses `Connection::recv_from()` (`src/network/networktransport.h:58,85-86`, `src/network/networktransport-impl.h:68-81`, `src/network/network.h:274-275`, `src/network/network.cc:590-600`) to receive only from that live socket and treats a pruned handle as no packet. The no-argument sweep remains unchanged. `test_readable_socket_identity()` (`win32/test_core.cc:89-142`) uses real loopback transport sockets, clears the new socket's `FD_READ` record, and asserts the same socket is drained through the public `MoshCore::on_readable(fd)` API; it is RED before the identity plumbing and GREEN afterward on native windows-11-arm CI.
...
* **Status:** **FIXED.** `SocketEvents` is now the production-visible internal class in `win32/socket_events.h:30-85`; `reconcile()` retains registrations for live handles but calls `WSAEventSelect(fd, event, FD_READ)` for each one (`win32/socket_events.h:59-78`), so a recycled raw handle is armed for its new socket. It preserves event creation, error cleanup, and the destructor's deregistration semantics. `test_recycled_socket_event_registration()` (`win32/test_core.cc:146-185`) creates real loopback UDP sockets, requires a recycled raw `SOCKET` value, reconciles it, and observes its `FD_READ` record through `WSAEnumNetworkEvents`; it is RED before the re-arm and GREEN afterward on native windows-11-arm CI.
