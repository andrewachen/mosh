...
#### I10. Configured SSH routes and connection sharing are refused outright

* **Upstream:** honors the user's `ProxyJump` and `ProxyCommand` except where its own `proxy` discovery method substitutes one (`scripts/mosh.pl:402`), and does not otherwise suppress connection sharing.
* **Port:** pins `ProxyJump=none`, `ProxyCommand=none`, and `-S none` on every invocation (`win32/mosh_bootstrap.cc:172`), so a destination reachable only through a jump host fails at `ssh` and an existing control master is never reused.
